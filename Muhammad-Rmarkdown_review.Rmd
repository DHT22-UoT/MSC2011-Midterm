...
```{r selena_review, include = FALSE}
# Grade: 10/10
# Overall, the report is very well organized and easy to follow. It has great structure, style, logic, performance, test coverage, design, readability, and functionality. It also fulfills all requirements. 
# I was able to knit without any errors, all necessary packages were explicitly loaded, the document was appropriately formatted using headers and text, the code outputs were properly formatted for printing/viewing, the outputs were disabled for appropriate chunks, and the code was chunked appropriately. The code chunks were named and options sets where necessary. The code was also commented well. I like how you use appropriate formatting of the text, such as making the figure legend bold where appropriate - it makes them stand out and easy to read. The only suggestion is to use more auto-generated tables instead of manually entered ones for the future, as any modifications to the code might alter the numbers and add more work to your plate. :)
```
