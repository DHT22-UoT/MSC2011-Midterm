...
Data cleaning and formatting were performed on the Trip and Weather datasets. Any outliers were removed. The final cleaned data were written as trip_clean.csv and weather_clean.csv and used for subsequent rush hour and correlation analysis.
...
Considering the broad range and high variation coefficient value of the Duration variable, potential outliers were removed based on the interquartile range (IQR). In this case, the upper and the lower limits were 1352.5 and -259.5, respectively. 
...
After cleaning the datasets, different functions were used to determine bike rental patterns for each day of the week. Using this output, frequent starting stations and ending stations were determined along with the average utilization of bikes for each month.
...
To determine the association between weather conditions and bike rental patterns, 
the associated datasets were merged using the inner join method in order to return a
data frame with all matched rows. First, the station dataset and the trip dataset 
were joined by the station id, which would construct a city column within the trip 
data. Next, the newly created data frame was joined with the weather dataset using 
dates and cities.
...
This dataframe would contain the duration of the trips along with each weather condition, from which correlations can be computed. All correlation coefficients between duration and the mean of each weather variable are illustrated in Table 7 and a graphical representation can be found in Figure 7 of the Appendix.
...
![**Figure 4.** Series of ggplots showing bike rental patterns at different hours of the day. Graph A represents the approximate pattern of all weekdays together. Graph B to graph F represent the bike rental pattern for each week day individually, Monday to Friday, respectively](ggplots.png)

![**Figure 5.** Graphical representation of the average utilization for each month, in 2014](avg_utilization.png)

![**Figure 6.** Correlation plot representing the relationship between trip duration and different weatherconditions. Scale on the right is the reference showing the strength of the relationship](corplot.png)

###Code review comments###
Rmd completeness - 2/2
Packages - 2/2
Text Formatting - 2/2
Output Formatting - 2/2 
Code chunks - 1/2 [code chunks are not set up properly]

9/10. Great job on the report Danni! I did notice that most of your code chunks followed the same defaults as the initial-setup chunk, so perhaps in your future reports, you could incorporate a mix of enabled and disabled code-view for code chunks that are not necessary to display in a report with the echo=False command. For example, your report's code for the boxplots in the 'weather_outliers' chunk could be disabled since the code for those figures were associated with the box-plots that were displayed later on in the Appendix of your report. Apart from that, I thought that the overall formatting of your report was super clear and easy to understand and your use of the ![FigCaption](Fig.name.jpg) syntax for selecting images you chose to include was super effective. Great job again! 
